...
export default async function ExploreCategoryPage({
	...
	const conditions = [
		...
		eq(prompts.category, categoryParam as Category),
	];
			...
			<main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-12">
				<div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
					<div className="max-w-2xl">
						<h1 className="text-4xl font-extrabold text-gray-900 tracking-tight mb-4 capitalize">
					...
					>
						<div className="relative w-full md:w-80">
							...
							<input
								...
								name="q"
								defaultValue={q}
								placeholder={`Search ${categoryParam.replace("_", " ")}...`}
				...
				<div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-12">
					<div className="flex flex-wrap items-center gap-2">
						<Link
							href={`/explore${q ? `?q=${q}` : ""}`}
							className="px-4 py-2 rounded-full text-sm font-semibold bg-white border border-gray-200/80 text-gray-600 hover:border-indigo-200 hover:text-indigo-600 shadow-sm transition-all"
						>
							All
						</Link>
						{CATEGORIES.map((cat) => (
							<Link
								...
								className={`px-4 py-2 rounded-full text-sm font-semibold shadow-sm transition-all capitalize ${
									cat === categoryParam
										? "bg-gray-900 text-white border-transparent"
										: "bg-white border border-gray-200/80 text-gray-600 hover:border-indigo-200 hover:text-indigo-600"
								}`}
							>
								{cat.replace("_", " ")}
							</Link>
					...
					<div className="flex items-center gap-2 bg-white border border-gray-200/80 rounded-xl p-1 shadow-sm w-full lg:w-auto overflow-x-auto">
						<Link
							href={`/explore/${categoryParam}?sort=newest${q ? `&q=${q}` : ""}`}
						...
						<Link
							href={`/explore/${categoryParam}?sort=popular${q ? `&q=${q}` : ""}`}
							className={`px-4 py-2 rounded-lg text-sm font-semibold transition-colors whitespace-nowrap ${
								sort === "popular"
									? "bg-gray-100 text-gray-900"
									: "text-gray-500 hover:text-gray-900 hover:bg-gray-50"
						...
						<Link
							href={`/explore/${categoryParam}?sort=price_asc${q ? `&q=${q}` : ""}`}
							className={`px-4 py-2 rounded-lg text-sm font-semibold transition-colors whitespace-nowrap ${
								sort === "price_asc"
									? "bg-gray-100 text-gray-900"
									: "text-gray-500 hover:text-gray-900 hover:bg-gray-50"
